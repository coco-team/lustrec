...
let rec add_expr_reset_cond cond expr =
  ...
  let new_desc = 
    ...
    | Expr_fby _ -> assert false (* TODO: deal with fby. This hasn't been much hadnled yet *)
...
let inline_call node loc uid args reset locals caller =
  ...
  let assign_inputs = 
    mkeq 
      loc 
      ( (* build a tuple assign, all at once *)
